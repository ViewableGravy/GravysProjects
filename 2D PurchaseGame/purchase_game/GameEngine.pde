...
public class GameEngine {
  ...
  final int INTERACT_DISTANCE = 45;
  ...
  //natively returns whether a player is within distance of the square of an entities radius (could change to an actual distance in future)
  boolean Range(Player plr, Entity entity) {
    return plr.Heading(entity.pos.x,entity.pos.y).magSq() < sq(INTERACT_DISTANCE);
  }
  ...
  Entity RequestInteraction(Player plr, ArrayList<Entity> entities){
    for (Entity entity : entities) {
      ...
      int DegreeToEntity = round(map(atan2(EntHeading.y, EntHeading.x), PI, -PI, -180, 180)) ; 
        if (plr.InteractDistanceEntity(DegreeToEntity,plr.Direction(mousex,mousey))) {
